    ...
    <div id="root">
        ...
        <main>
            <div class="container">
                    ...
                    <div class="row">
                        ...
                        <div class="col-auto mb-3 mb-md-0 me-auto">
                            <div class="w-auto sw-md-70">
                                <a href="./admin.php" class="muted-link pb-1 d-inline-block breadcrumb-back">
                                    <i data-cs-icon="chevron-left" data-cs-size="13"></i>
                                    <span class="text-small align-middle">Администраторы</span>
                                </a>
                                <h1 class="mb-0 pb-0 display-4" id="title"><?php echo empty($_GET['edit']) ? 'Новый администратор' : 'Редактирование администратора №' . $_GET['edit']; ?></h1>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                if (isset($_POST['save'])) {
                    if (empty($_GET['edit'])) {
                        $sql = "INSERT INTO `admin` (username, password, arole) VALUES (?, ?, ?)";
                        $stmt = mysqli_prepare($conn, $sql);
                        mysqli_stmt_bind_param($stmt, "ssi", $_POST['username'], md5($_POST['password']), $_POST['arole']);
                        mysqli_stmt_execute($stmt);
                    } else {
                        $sql = "UPDATE `admin` SET username=?, password=?, arole=? WHERE aid=" . $_GET['edit'];
                        $stmt = mysqli_prepare($conn, $sql);
                        $stmt->bind_param("ssi", $_POST['username'], md5($_POST['password']), $_POST['arole']);
                        $stmt->execute();
                    }
                }
                ?>
                    ...
                    <div class="col-12 col-xl-10 col-xxl-12">
                        ...
                        <div class="card">
                            <div class="card-body mb-n5">
                                <div class="mb-5">
                                    <div>
                                        <p class="text-small text-muted mb-2">Данные об администраторе</p>
                                        <div class="row g-0 mb-2">
                                            <form id="<?php echo $idAttr; ?>" rel="<?php echo $editId; ?>" name="author_profile" method="POST">
                                                ...
                                                <div class="form-group m-2">
                                                    <label for="arole" class="mb-1">
                                                        Роль
                                                    </label>
                                                    <br />
                                                    <select class="form-control" name="arole" id="arole">
                                                        <option value="0">Администратор</option>
                                                        <option value="1">Аудит</option>
                                                    </select>
                                                </div>
