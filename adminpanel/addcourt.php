...
$editId = $_GET['edit'];
if (!empty($editId)) {
    $sqlEditInputs = 'select ctid, kname from `court` where kid=' . $editId;
    ...
    <div id="root">
        ...
        <main>
            <div class="container">
                    ...
                    <div class="row">
                        ...
                        <div class="col-auto mb-3 mb-md-0 me-auto">
                            <div class="w-auto sw-md-70">
                                ...
                                <h1 class="mb-0 pb-0 display-4" id="title"><?php echo empty($_GET['edit']) ? 'Новый корт' : 'Редактирование корта №' . $_GET['edit']; ?></h1>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                if (isset($_POST['save'])) {
                    if (empty($_GET['edit'])) {
                        $sql = "INSERT INTO `court` (ctid, kname, ktype, kopenness) VALUES (?, ?, ?, ?)";
                        $stmt = mysqli_prepare($conn, $sql);
                        mysqli_stmt_bind_param($stmt, "isss", $_POST['ctid'], $_POST['kname'], $_POST['ktype'], $_POST['kopenness']);
                        mysqli_stmt_execute($stmt);
                    } else {
                        $sql = "UPDATE `court` SET ctid=?, kname=?, ktype=?, kopenness=? WHERE kid=" . $_GET['edit'];
                        $stmt = mysqli_prepare($conn, $sql);
                        $stmt->bind_param("isss", $_POST['ctid'], $_POST['kname'], $_POST['ktype'], $_POST['kopenness']);
                        $stmt->execute();
                    }
                }
                ?>
                    ...
                    <div class="col-12 col-xl-10 col-xxl-12">
                        ...
                        <div class="card">
                            <div class="card-body mb-n5">
                                <div class="mb-5">
                                    <div>
                                        <p class="text-small text-muted mb-2">Данные о корте</p>
                                        <div class="row g-0 mb-2">
                                            <form id="<?php echo $idAttr; ?>" rel="<?php echo $editId; ?>" name="author_profile" method="POST">
                                                <div class="form-group m-2">
                                                    ...
                                                    <br />
                                                    <select class="form-control" name="ctid" id="ctname">
                                                        <?php
                                                        if ($_SESSION['role'] != 2)
                                                            $sqlSelectCategory = "select ctid, ctname from `center`";
                                                        else {
                                                            ...
                                                            $sqlSelectCategory = "select ctid, ctname from `center` where ctid = " . $datacenterrole['ctid'];
                                                        }
                                                            
                                                        $resSelectCategory = $conn->query($sqlSelectCategory);
                                                        while ($dataSelectCategory = mysqli_fetch_assoc($resSelectCategory)) {
                                                        ?>
                                                            <option value="<?php echo $dataSelectCategory['ctid']; ?>"><?php echo $dataSelectCategory['ctname']; ?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                                <div class="form-group m-2">
                                                    ...
                                                    <input type="text" name="kname" class="form-control" id="exampleFirstName1" placeholder="Имя" value="<?php echo $dataEditInputs['kname'] ?>" required>
                                                </div>
                                                <div class="form-group m-2">
                                                    <label for="ktype" class="mb-1">
                                                        Тип тенниса
                                                    </label>
                                                    <br />
                                                    <select class="form-control" name="ktype" id="ktype">
                                                        <option value="Теннис">Теннис</option>
                                                        <option value="Настольный теннис">Настольный теннис</option>
                                                        <option value="Сквош">Сквош</option>
                                                    </select>
                                                </div>
                                                <div class="form-group m-2">
                                                    <label for="kopenness" class="mb-1">
                                                        Тип корта
                                                    </label>
                                                    <br />
                                                    <select class="form-control" name="kopenness" id="kopenness">
